...
import edu.ser.pivi.mindmap.diagram.part.PiviDiagramEditor;
		...
		public final void selectionChanged(IStructuredSelection selection) {
			...
			if (selection.size() == 1) {
				...
				if (selectedElement instanceof PiviNavigatorItem) {
					selectedElement = ((PiviNavigatorItem) selectedElement).getView();
				} else if (selectedElement instanceof IAdaptable) {
					selectedElement = ((IAdaptable) selectedElement).getAdapter(View.class);
				}
				if (selectedElement instanceof Diagram) {
					Diagram diagram = (Diagram) selectedElement;
					if (MapEditPart.MODEL_ID.equals(PiviVisualIDRegistry.getModelID(diagram))) {
						myDiagram = diagram;
					}
				}
			}
		...
		*/
		public void run() {
			...
			try {
				...
				PiviDiagramEditorPlugin.getInstance().logError("Exception while openning diagram", e); //$NON-NLS-1$
			}
		}
