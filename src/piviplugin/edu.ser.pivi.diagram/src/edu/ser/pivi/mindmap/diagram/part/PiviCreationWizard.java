...
public class PiviCreationWizard extends Wizard implements INewWizard {
	...
	*/
	public void init(IWorkbench workbench, IStructuredSelection selection) {
		...
		setDefaultPageImageDescriptor(
				PiviDiagramEditorPlugin.getBundledImageDescriptor("icons/wizban/NewMindmapWizard.gif")); //$NON-NLS-1$
	...
	*/
	public void addPages() {
		diagramModelFilePage = new PiviCreationWizardPage("DiagramModelFile", getSelection(), "mindmap_diagram"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		domainModelFilePage = new PiviCreationWizardPage("DomainModelFile", getSelection(), "mindmap") { //$NON-NLS-1$ //$NON-NLS-2$

			public void setVisible(boolean visible) {
				if (visible) {
					String fileName = diagramModelFilePage.getFileName();
					fileName = fileName.substring(0, fileName.length() - ".mindmap_diagram".length()); //$NON-NLS-1$
					setFileName(PiviDiagramEditorUtil.getUniqueFileName(getContainerFullPath(), fileName, "mindmap")); //$NON-NLS-1$
				}
