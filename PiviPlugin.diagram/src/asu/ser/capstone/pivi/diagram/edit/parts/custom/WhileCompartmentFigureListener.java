...
public class WhileCompartmentFigureListener implements FigureListener {
	...
	public WhileCompartmentFigureListener(ListCompartmentEditPart compartmentEditPart,
			...
			for (AbstractEditPart child : childs) {
				if (child instanceof AbstractGraphicalEditPart) {
					...
					// Operator input ?
						...
						constraint = new Rectangle((int) (MARGIN * xScale), !firstInputProcessed
								? (int) ((MARGIN + R) * yScale) : (int) ((MARGIN + R * 3) * yScale), (int) (R * xScale),
								(int) (R * yScale));
						contentPane.setConstraint(gEditPart.getFigure(), constraint);
