...
public class StatementCompartmentFigureListener implements FigureListener{

	private ListCompartmentEditPart compartmentEditPart = null;
	private RoundedRectangle roundedRectangle = null;
 
	public static final double MARGIN = 30; // The margin to apply before drawing our operator
	public static final double R = 70; // The base length
	...
	public StatementCompartmentFigureListener(ListCompartmentEditPart compartmentEditPart, RoundedRectangle roundedRectangle) {
				...
				if (child instanceof AbstractGraphicalEditPart) {
					...
					if (gEditPart instanceof InputPortEditPart) {
						constraint = new Rectangle(
								(int) (MARGIN * xScale),
								(int) ((REF_H - (3 * R) - MARGIN) * yScale),
								(int) (R * xScale), 
								(int) (R * yScale));
						contentPane.setConstraint(gEditPart.getFigure(), constraint);
					}
					
					else if (gEditPart instanceof ResultEditPart) {
						constraint = new Rectangle(
								(int) (MARGIN * xScale),
								(int) ((REF_H - R - MARGIN) * yScale),
								(int) (R * xScale), 
								(int) (R * yScale));
						contentPane.setConstraint(gEditPart.getFigure(), constraint);
					}
